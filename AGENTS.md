...
- Report capture now logs early case allocation and persists top-level `minimize_status` in `summary.json` (`disabled` / `not_applicable` / `pending` / `success` / `skipped`) for better logs/reports consistency (2026-02-07).
