...
- Added DQP hint support for `SET_VAR(tidb_opt_partial_ordered_index_for_topn='COST'|'DISABLE')`, introduced config-driven external DQP hint injection via `oracles.dqp_external_hints` (supports full hint strings and raw `var=value` SET_VAR shorthand), and documented usage in README; validated with `go test ./internal/oracle`, `go test ./internal/config`, and `go test ./cmd/shiro` using Go 1.25.7 (2026-02-15).
