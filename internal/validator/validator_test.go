...
func TestValidateLateralMergedColumnSQL(t *testing.T) {
	v := New()
	sql := "SELECT id AS merged_id, dt.id AS lateral_id FROM t0 JOIN t1 USING (id) JOIN LATERAL (SELECT src.v0 AS id FROM (SELECT t2.c2 AS v0 FROM t2) AS src WHERE (src.v0 = id)) AS dt ON (1 = 1) ORDER BY 1, 2"
	if err := v.Validate(sql); err != nil {
...
func TestValidateLateralGroupedOutputOrderLimitSQL(t *testing.T) {
	v := New()
	sql := "SELECT id AS merged_id, dt.g0 AS lateral_g0, dt.cnt AS lateral_cnt FROM t0 JOIN t1 USING (id) JOIN LATERAL (SELECT src.v0 AS g0, COUNT(1) AS cnt FROM (SELECT t2.c2 AS v0 FROM t2) AS src WHERE (src.v0 <> id) GROUP BY src.v0 HAVING (ABS(CASE WHEN (src.v0 >= id) THEN (COUNT(1) - id) ELSE ((COUNT(1) + src.v0) - id) END) >= 1) ORDER BY g0, cnt DESC, id LIMIT 1) AS dt ON (1 = 1) ORDER BY 1, 2, 3"
	if err := v.Validate(sql); err != nil {
...
func TestValidateLateralProjectedOrderLimitSQL(t *testing.T) {
	v := New()
	sql := "SELECT id AS merged_id, dt.score0 AS lateral_score0, dt.tie0 AS lateral_tie0 FROM t0 JOIN t1 USING (id) JOIN LATERAL (SELECT ABS(CASE WHEN (src.v0 >= id) THEN CASE WHEN (src.v0 >= 0) THEN (src.v0 - id) ELSE id END ELSE CASE WHEN (src.v0 >= 0) THEN (id - src.v0) ELSE src.v0 END END) AS score0, ABS(CASE WHEN (src.v0 >= 0) THEN (src.v0 + id) ELSE (id - src.v0) END) AS tie0 FROM (SELECT t2.c2 AS v0 FROM t2) AS src WHERE (src.v0 <> id) ORDER BY score0, tie0 DESC, id LIMIT 1) AS dt ON (1 = 1) ORDER BY 1, 2, 3"
	if err := v.Validate(sql); err != nil {
